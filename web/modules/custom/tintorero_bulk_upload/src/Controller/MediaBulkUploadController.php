...
namespace Drupal\tintorero_bulk_upload\Controller;
...
class MediaBulkUploadController extends ControllerBase
{
  ...
  public function addLinksList()
  {
    ...
    $entity_type = $this->entityTypeManager->getDefinition('tintorero_bulk_config');
    ...
    $mediaBulkConfigStorage = $this->entityTypeManager->getStorage('tintorero_bulk_config');
    $mediaBulkConfigEntities = $mediaBulkConfigStorage->loadMultiple();

    $form_route_name = 'tintorero_bulk_upload.upload_form';
    if (count($mediaBulkConfigEntities) == 1) {
      $mediaBulkConfigEntity = reset($mediaBulkConfigEntities);
      return $this->redirect($form_route_name, ['tintorero_bulk_config' => $mediaBulkConfigEntity->id()]);
    }

    foreach ($mediaBulkConfigEntities as $mediaBulkConfigEntity) {
      $link = Link::createFromRoute($mediaBulkConfigEntity->label(), $form_route_name, ['tintorero_bulk_config' => $mediaBulkConfigEntity->id()]);
      ...
      $build['#bundles'][$mediaBulkConfigEntity->id()] = [
        ...
        'description' => '',
        'add_link' => Link::createFromRoute($mediaBulkConfigEntity->label(), $form_route_name, ['tintorero_bulk_config' => $mediaBulkConfigEntity->id()]),
      ];
    }
  ...
  {
    if ($account->hasPermission('administer tintorero_bulk_upload configuration')) {
    ...
    foreach ($mediaBulkConfigEntities as $mediaBulkConfig) {
      $url = Url::fromRoute('tintorero_bulk_upload.upload_form', ['tintorero_bulk_config' => $mediaBulkConfig->id()]);
      if ($url->access()) {
        return AccessResult::allowed();
      }
    }
   ...
   * @param \Drupal\Core\Session\AccountInterface $account
   *   User to validate access on.
   * @param \Drupal\tintorero_bulk_upload\Entity\MediaBulkConfigInterface $tintorero_bulk_config
   *   The media bulk config entity the upload form belongs to.
   *
   * @return \Drupal\Core\Access\AccessResultAllowed|\Drupal\Core\Access\AccessResultForbidden
   */
  public function accessForm(AccountInterface $account, MediaBulkConfigInterface $tintorero_bulk_config)
  {
    $mediaBulkConfigId = $tintorero_bulk_config->id();
